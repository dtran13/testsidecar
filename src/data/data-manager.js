...
(function(app) {
    ...
    var _dataManager = _.extend({
        ...
        getSyncCallbacks: function(method, model, options) {
            var success  = this.getSyncSuccessCallback(method, model, options);
            var error    = this.getSyncErrorCallback(model, options);
            var complete = this.getSyncCompleteCallback(method, model, options, error.error)
        ...
        getSyncErrorCallback: function(model, options) {
            var err;
            ...
            return { error: err, callback: error };
        },

        getSyncCompleteCallback: function(method, model, options, error) {
            var self = this;

            // 'complete' fires after success and error
            var complete = function() {
                // trigger global data:sync:end event
                 ...
                 * <pre><code>
                 * (function(app) {
                 *     model.on("data:sync:end", function(method, options, error) {
                 *         app.logger.debug("Finished operation " + method + " on " + model);
                 *     });
                 * })(SUGAR.App);
                 * </code></pre>
                 * @event
                 */
                model.trigger("data:sync:end", method, options, error);
